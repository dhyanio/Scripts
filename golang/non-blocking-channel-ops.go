...
func main() {
	...
	// Here’s a non-blocking receive. If a value is available on messages then select will take
	// the <-messages case with that value. If not it will immediately take the default case.
    select {
    case msg := <-messages:
